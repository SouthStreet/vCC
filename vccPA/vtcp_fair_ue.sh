...
if [ "$#" -lt 1 ]
then
  echo "Usage: `basename $0` key bw bwlow bwhigh cutoff vtcp label limit min max avpkt burst prob" 1>&2
  exit 1
fi
...
qsize=1000
    ...
    rm -rf $dir
    mkdir -p $dir
    python tcpfair_unequal.py --cong reno --cong1 reno --congrest reno --delay $delay -b $bwnet -Bl $bwlow -Bh $bwhigh -d $dir --maxq $qsize -t $time \
    --red_limit $red_limit \
    --red_min $red_min \
    --red_max $red_max \
    --red_avpkt $red_avpkt \
    ...
    ./process $warmup $maxtime $nshosts $cutoff $key1 $dir/${trace##*/}.gz
    chown -R li $dir/*
    pkill -9 iperf
